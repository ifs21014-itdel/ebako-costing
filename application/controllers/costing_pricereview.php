...
class costing_pricereview extends CI_Controller {
    ...
    function search_pricereview($offset = 0) { //tanda
        ...
        foreach ($list_costing as $costing) {
            ...
            if ($con == 'c1') {
                ...
                $costing_review["rate_value"] = $costing->ratevalue;
                $costing_review["pick_list_rate_value"] = $costing->picklist_ratevalue;
            } else if ($con == 'c2') {
                ...
                $costing_review["rate_value"] = $ratevalue;
                $costing_review["pick_list_rate_value"] = $picklist_ratevalue;
            } else if ($con == 'c4') {
            ...
            $costing_details["title1"] = $result['title1'];
            $costing_details["title2"] = $result['title2'];

            $costing_review["costing_id"] = $costing->id;
            $costing_review["fob_price"] = $result['fob_new'];
            $costing_review["margin"] = $result['new_result'];
            $costing_review["customer_id"] = $costing->customerid;
            $costing_review["model_id"] = $costing->modelid;
            $costing_review["year"] = date("Y");
            $costing_review["old_fob"] = $result['fob_old'];

            $this->model_costing_review->create($costing_review);
        }
        ...
        $costing_details["title1"] = isset($costing_details['title1']) ? $costing_details['title1'] : "Profit (%)";
        $costing_details["title2"] = isset($costing_details['title2']) ? $costing_details['title2'] : "FOB";
    ...
    function currencyConditionalFixPriceMarginConditional($ratevalue, $margin, $costing, $picklist_ratevalue) {
        ...
        $old_fob = round($subtotal_old + $factory_cost_and_profit_old, 2);

        return array(
            'old_result' => round($old_fob, 2),
            'new_result' => round($new_fob, 2),
            'fob_old' => round($old_fob, 2),
            'fob_new' => round($new_fob, 2),
            'title1' => "Old FOB",
            'title2' => "New FOB",
        );
    }
    ...
    function getMargin($costing, $ratevalue = null, $picklist_ratevalue = null) {
        ...
        $fob_price_new = round($subtotal_new + $factory_cost_and_profit_new, 2);
        $fob_price_old = round($subtotal_old + $factory_cost_and_profit_old, 2);
        ...
        $margin = array(
            'old_result' => round($old_margin, 2),
            'new_result' => round($new_margin, 2),
    ...
    function convert_price($item, $ratevalue, $costingdetail) {
        // Cek apakah $item adalah objek
        if (!is_object($item)) {
            return 0; // Atau nilai default yang sesuai
        }

        // Cek apakah properti $item->curr ada, jika tidak, set ke 'IDR'
        if (!isset($item->curr)) {
            $item->curr = 'IDR';
        }

        // Jika currency adalah IDR, lakukan konversi
        if ($item->curr == 'IDR') {
            return $item->price / $ratevalue;
        }

        // Jika bukan IDR, kembalikan harga asli
        return $item->price;
    }
}
