...
class model_model extends CI_Model {
    ...
    function selectById($id) {
        $query = "select model.*,customer.name customername ,
					...
					(select name as approvedby_name from employee where employee.id=model.approvedby) 
                                        from model left join customer on model.customerid=customer.id where model.id=" . $id;
        //echo $query."<br/>";
    ...
    function getNumRows($code, $description, $modeltypeid, $custcode) {
        ...
        $query .= " and model.ishavebom=true and model.checkedstatus='1' and model.approvedstatus='1' order by model.id desc";
        return $this->db->query($query)->num_rows();
    }

    function search($code, $description, $modeltypeid, $custcode, $limit, $offset) {
        $customerid = $this->input->post('customerid');
        $approvedstatus = $this->input->post('status_search_id');
        $query = "select model.*,modeltype.name modeltypename ,customer.name customername , 
                (select name as checkedby_name from employee where employee.id=model.checkedby), 
                (select name as approvedby_name from employee where employee.id=model.approvedby) from model 
                join modeltype on model.modeltypeid=modeltype.id LEFT join customer on model.customerid=customer.id where true 
                 ";
        if ($code != "") {
            $query .= " and model.no ilike '%" . $code . "%'";
        }if ($description != "") {
            ...
            $query .= " and model.customerid =" . $customerid;
        }if ($custcode != "") {
            $query .= " and model.custcode ilike '%$custcode%'";
        }if ($approvedstatus != "All") {
            if ($approvedstatus == "") {
                $query .= " and model.approvedstatus is null ";
            } else
                $query .= " and model.approvedstatus='$approvedstatus'";
        }
    ...
    function searchAvailabel($code, $description, $modeltypeid, $custcode, $limit, $offset) {
        $query = "select model.*,modeltype.name modeltypename from model join
                                    modeltype on model.modeltypeid=modeltype.id where true";
        if ($code != "") {
            $query .= " and model.no ilike '%" . $code . "%'";
        }if ($description != "") {
            $query .= " and model.description ilike '%" . $description . "%'";
