Vue.component('undirectmaterial', {
    props: ['material', 'category', 'index_costingundirectmaterial', 'index_costingundirectmaterial_datas'],
    data() {
        return {
            req_qty: 0,
            unitpriceusd: 0,
    ...
    created: function () {
        ...
        if (this.material.yield == 0) { //avoid invinity
            this.material.yield = 0;
        }
        if (this.material.allowance == 0) {
...
//                if(this.material.categoryid==8)
//                      alert(this.material.yield);
...
//            this.material.req_qty = this.req_qty;
//
//            var req_qty = roundTo3(this.material.qty / this.material.yield * (1 + this.material.allowance))
            return this.req_qty;
        },
    ...
    methods: {
        ...
        init_unitprice_rp_usd: function () {
            ...
            var ratevalue = this.$root.costing.ratevalue;
            if (this.category.id == '8') {
                ratevalue = this.$root.costing.picklist_ratevalue;
            }

            if (unitpricerp <= 0 && unitpriceusd > 0) {
                unitpricerp = unitpriceusd * parseFloat(ratevalue);
                unitpricerp = roundTo3(unitpricerp);
                this.material.unitpricerp = unitpricerp;
            } else if (unitpricerp > 0 && unitpriceusd <= 0) {
                unitpriceusd = unitpricerp / parseFloat(ratevalue);
                unitpriceusd = roundTo3(unitpriceusd);
                this.material.unitpriceusd = unitpriceusd;
            }
        ...
        calculate_unitpricerp: function () {
            ...
            if (this.new_curr_costing_price == "" || this.new_curr_costing_price == 'IDR') {
                ...
                var unitpricerp = parseFloat(this.material.unitpriceusd) * parseFloat(ratevalue);
        ...
        calculate_unitpriceusd: function () {
            ...
            if (this.new_curr_costing_price == "" || this.new_curr_costing_price == 'USD') {
                ...
                var unitpriceusd = parseFloat(this.material.unitpricerp) / parseFloat(ratevalue);
                ...
                axios({
                    ...
                    data: "costingdetail_id=" + this.material.id,
                        ...
                        .then(response => {
                            ...
                            this.$root.costingundirectmaterial[ this.index_costingundirectmaterial ].datas.splice(this.index_costingundirectmaterial_datas, 1, null);
    ...
    template: `
  			<tr class="m-row" v-bind:style="{ backgroundColor: (material.source=='Manual' ? '#ffeb3b':'#fff') }">
				...
				<td align="right" style="border: 1px solid black;">
					<input v-if="! this.$root.is_costing_locked" class="input-editable" style="width:100%;" type="text" v-model="material.qty" v-on:change="onChangeQty" :key="material.id + '_qty_undirect'" title="QTY based on BOM"></input>
					<label v-if="this.$root.is_costing_locked">{{material.qty | float3decimalPoint }}</label>
				</td>
				<td align="center" style="border: 1px solid black;">
					<input v-if="! this.$root.is_costing_locked && category.id !='9' " class="input-editable" style="width:100%;" type="text" v-model="material.yield" v-on:change="onChangeYield" :key="material.id + '_yield_undirect'" title="Yield"></input>
					<label v-if="this.$root.is_costing_locked">{{material.yield}}</label>
					
					<span v-if="(!this.$root.is_costing_locked) && category.id =='9'">{{material.yield}}</span>
					
				</td>
				<td align="center" style="border: 1px solid black;">
					<input v-if="! this.$root.is_costing_locked" class="input-editable" style="width:100%;" type="text" v-model="material.allowance" v-on:change="onChangeAllowance" :key="material.id + '_allowance_undirect'" title="Allowance"></input>
				...
				<td align="right" style="border: 1px solid black;">
					<input v-if="! this.$root.is_costing_locked" class="input-editable" style="width:100%;" type="text" v-model="material.unitpricerp" v-on:change="onChangeUnitpricerp" v-on:keydown="onKeydownUnitpricerp" :key="material.id + '_unitpricerp_undirect'" title="Unit Price Rp"></input>
					<label v-if="this.$root.is_costing_locked">{{material.unitpricerp | float3decimalPoint}}</label>
				</td>
				<td align="right" style="border: 1px solid black;">
					<input v-if="! this.$root.is_costing_locked" class="input-editable" style="width:100%;" type="text" v-model="material.unitpriceusd" v-on:change="onChangeUnitpriceusd" v-on:keydown="onKeydownUnitpriceusd" :key="material.id + '_unitpriceusd_undirect'" title="Unit Price USD"></input>
				...
				<td align="center" style="border: 1px solid black; text-align: left; vertical-align: middle;">
					<div v-if="! this.$root.is_costing_locked">
						...
						<button style="width: 40%;" v-if="! is_show_form_move_material" class="btn btn-xs button-delete-costing-detail" v-on:click="deleteCostingDetail" :key="'delete_material_' + category.id" title="Delete"> X </button>
					  	<button style="width: 40%;" v-if="! is_show_form_move_material && category.id != '9' " class="btn btn-xs button-move-costing-detail" v-on:click="show_form_move_material" :key="'show_form_move_material_' + category.id" title="Move">Move</button>
					 </div>
				</td>
			</tr>	
	`,
